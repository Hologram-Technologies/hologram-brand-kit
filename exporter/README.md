...
Every key is namespaced with the tenant (`PENPOT_TENANT`, `default` in code but
set to the workspace name in devenv, e.g. `devenv-ws0`), matching the prefix
used for pub/sub topics.

```
{tenant}.export.job.{job-id}   hash    field: data (transit blob of the whole
                                       record)
{tenant}.export.job-cancel     pubsub  payload: the job id, one line
```
...
(`PENPOT_EXPORT_JOB_TTL`, default 3600s), refreshed on every write and never
...
Keys are prefixed with the tenant, which in devenv is the **workspace name**
(`$PENPOT_TENANT`, e.g. `devenv-ws0`), not `default`. From the prompt:

```
# every job record
KEYS devenv-ws0.export.job.*

# the whole record, transit-json in the `data` field
HGET devenv-ws0.export.job.<job-id> data

# seconds left before the record expires
TTL devenv-ws0.export.job.<job-id>

# watch cancellations as they are published (blocks the connection)
SUBSCRIBE devenv-ws0.export.job-cancel

# drop one record
DEL devenv-ws0.export.job.<job-id>
```

`KEYS` is fine here -- the keyspace is a handful of job hashes. On a real
deployment use `SCAN 0 MATCH <tenant>.export.job.* COUNT 100` instead. Do not
`FLUSHDB`: the backend shares this database.
...
| `PENPOT_EXPORT_JOB_TTL`               | `3600`  | Lifetime of a job record, in seconds  |
| `PENPOT_EXPORT_MAX_CONCURRENT_JOBS`   | `4`     | Admission limit                       |
| `PENPOT_EXPORT_MAX_JOBS_PER_PROFILE`  | `2`     | Per-profile admission limit           |
| `PENPOT_EXPORT_QUEUE_MAX`             | `64`    | Queue cap; over it, `429 :queue-full` |
